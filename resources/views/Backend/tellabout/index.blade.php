          ...
          <div class="panel">
            <div class="panel-body">
              <div class="row">
                <div class="col-sm-6">
                  <div class="m-b-30">
                    <a href="{{ route('tellabout.create') }}" id="addToTable" class="btn btn-primary waves-effect waves-light">افزودن <i class="fa fa-plus"></i></a>
                  </div>
                </div>
              </div>
                      ...
                      <table class="table table-striped dataTable no-footer" id="datatable-editable" role="grid" aria-describedby="datatable-editable_info">
                        ...
                        <tbody>
                          ...
                          <tr class="gradeX odd" role="row">
                            ...
                            <td>{{ $tellabout->role}} </td>
                            <td>{{ $tellabout->name}} </td>
                            <td> <img src="{{ URL::to($tellabout->image) }}" height="50px;" width="50px;"> </td>
                            <td>@if($tellabout->status == 1) فعال @else غیر فعال @endif</td>
                            <td>{{ \Carbon\Carbon::parse($tellabout->created_at)->diffForhumans()  }}</td>

                            <td class="actions">

                              <a href="{{ route('tellabout.edit',$tellabout->id) }}" class="on-default edit-row"><i class="fa fa-pencil"></i></a>
                              <form id="delete-form-{{ $tellabout->id }}" method="post" action="{{ route('tellabout.destroy',$tellabout->id) }}" style="display: none">
                                {{ csrf_field() }}
                                {{ method_field('DELETE') }}
                              </form>
                              ...
                              if(confirm('Are you sure, You Want to delete this?'))
                                  {
                                      ...
                                      document.getElementById('delete-form-{{ $tellabout->id }}').submit();
                                  }
                                  else{
                                      event.preventDefault();
