                                            ...
                                            <table class="table table-striped dataTable no-footer" id="datatable-editable" role="grid" aria-describedby="datatable-editable_info">
                                                ...
                                                <tbody>
                                                    ...
                                                    <tr class="gradeX odd" role="row">
                                                        ...
                                                        <td>{{ $row->last_name}} </td>
                                                        <td>@if($row->subject == 1)مسابقات @elseif($row->subject == 2)کتابها @elseif($row->subject == 3)جوایز @else($row->subject == 4)همه موارد @endif</td>
                                                        <td>{{ $row->email}}</td>
                                                        <td>{{ \Carbon\Carbon::parse($row->created_at)->diffForhumans()  }}</td>
                                                        <td class="actions">

                                                            <form id="delete-form-{{ $row->id }}" method="get" action="{{ route('contact.destroy',$row->id) }}" style="display: none">
                                                                {{ csrf_field() }}
                                                                {{ method_field('DELETE') }}
                                                            </form>
