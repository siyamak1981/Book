              ...
              <div class="row">
                <div class="col-sm-6">
                  <div class="m-b-30">
                    ...
                    <a href="{{ route('torist_book.create') }}" id="addToTable" class="btn btn-primary waves-effect waves-light">افزودن <i class="fa fa-plus"></i></a>
                      ...
                      <table class="table table-striped dataTable no-footer" id="datatable-editable" role="grid" aria-describedby="datatable-editable_info">
                        ...
                        <tbody>
                          ...
                          <tr class="gradeX odd" role="row">
                            ...
                            <td>{!! htmlspecialchars_decode($toristbook->body) !!}</td>
                            <td><img src="{{Storage::disk('local')->url('toristbook/'.$toristbook->image)}}" width=50></td>
                            <td>@if($toristbook->status) فعال @else غیر فعال @endif</td>
                            <td>{{ \Carbon\Carbon::parse($toristbook->StartDate)->diffForhumans()  }}</td>
                            <td class="actions">
                              <a href="{{ route('torist_book.edit',$toristbook->id) }}" class="on-default edit-row"><i class="fa fa-pencil"></i></a>

               
                              <form id="delete-form-{{ $toristbook->id }}" method="post" action="{{ route('torist_book.destroy',$toristbook->id) }}" style="display: none">
                                {{ csrf_field() }}
                                {{ method_field('DELETE') }}
                              </form>
                                                            ...
                                                            if(confirm('Are you sure, You Want to delete this?'))
                                                                {
                                                                    ...
                                                                    document.getElementById('delete-form-{{ $toristbook->id }}').submit();
                                                                }
                                                                else{
                                                                    event.preventDefault();
