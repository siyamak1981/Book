              ...
              <div class="row">
                <div class="col-sm-6">
                  <div class="m-b-30">
                    ...
                    <a href="{{ route('competition.create') }}" id="addToTable" class="btn btn-primary waves-effect waves-light">افزودن <i class="fa fa-plus"></i></a>
                      ...
                      <table class="table table-striped dataTable no-footer" id="datatable-editable" role="grid" aria-describedby="datatable-editable_info">
                        ...
                        <tbody>
                          ...
                          <tr class="gradeX odd" role="row">
                            ...
                            <td>{!! htmlspecialchars_decode($competition->body) !!}</td>
                            <td><img src="{{URL::to($competition->image)}}" width=50></td>
                            <td>@if($competition->status) فعال @else غیر فعال @endif</td>
                            <td>{{ \Carbon\Carbon::parse($competition->StartDate)->diffForhumans()  }}</td>
                            <td class="actions">
                              <a href="{{ route('competition.edit',$competition->id) }}" class="on-default edit-row"><i class="fa fa-pencil"></i></a>

               
                              <form id="delete-form-{{ $competition->id }}" method="post" action="{{ route('competition.destroy',$competition->id) }}" style="display: none">
                                {{ csrf_field() }}
                                {{ method_field('DELETE') }}
                              </form>
