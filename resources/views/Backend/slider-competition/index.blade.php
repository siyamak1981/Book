              ...
              <div class="row">
                <div class="col-sm-6">
                  <div class="m-b-30">
                    ...
                    <a href="{{ route('slider-competition.create') }}" id="addToTable" class="btn btn-primary waves-effect waves-light">افزودن <i class="fa fa-plus"></i></a>
                      ...
                      <table class="table table-striped dataTable no-footer" id="datatable-editable" role="grid" aria-describedby="datatable-editable_info">
                        ...
                        <tbody>
                          ...
                          <tr class="gradeX odd" role="row">
                            ...
                            <td>{{ $slidercompetition->title}} </td>
                            <td>{!! htmlspecialchars_decode($slidercompetition->body) !!}</td>
                            <td><img src="{{ URL::to($slidercompetition->image) }}" width=50></td>
                            <td>@if($slidercompetition->status) فعال @else غیر فعال @endif</td>
                            <td>{{ \Carbon\Carbon::parse($slidercompetition->created_at)->diffForhumans() }}</td>
                            <td class="actions">
                              <a href="{{ route('slider-competition.edit',$slidercompetition->id) }}" class="on-default edit-row"><i class="fa fa-pencil"></i></a>
                              <form id="delete-form-{{ $slidercompetition->id }}" method="post" action="{{ route('slider-competition.destroy',$slidercompetition->id) }}" style="display: none">
                                {{ csrf_field() }}
                                {{ method_field('DELETE') }}
                              </form>
                                                            ...
                                                            if(confirm('Are you sure, You Want to delete this?'))
                                                                {
                                                                    ...
                                                                    document.getElementById('delete-form-{{ $slidercompetition->id }}').submit();
                                                                }
                                                                else{
                                                                    event.preventDefault();
